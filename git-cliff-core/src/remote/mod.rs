...
fn create_remote_client(remote: &Remote, accept_header: &str) -> Result<ClientWithMiddleware> {
    if !remote.is_set() {
    ...
    let client = ClientBuilder::new(client)
        .with(Cache(HttpCache {
            ...
            manager: CACacheManager {
                path: dirs::cache_dir()
                    .ok_or_else(|| {
                        Error::DirsError(String::from("failed to find the user's cache directory"))
                    })?
